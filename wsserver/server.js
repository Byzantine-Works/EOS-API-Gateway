...
app.listen(process.env.WS_SOCKET_SERVER_PORT, () => console.log('socket.io Server Listening on ', process.env.WS_SOCKET_SERVER_PORT));
io.listen(process.env.WS_SOCKET_SERVER_LISTENER_PORT, {
  timeout: process.env.WS_SOCKET_TIMEOUT
});
...
io.on(process.env.WS_SOCKET_CHANNEL_ONCONNECT, (client) => {
  ...
  //TODO @Victor pass api_key to ensure this method has sla/anonymous usage boundaries
  ...
  client.on(process.env.WS_SOCKET_CHANNEL_TRANSAC_STATUS, async (data) => {
    ...
    let int = setInterval(reqTrans, process.env.WS_SOCKET_TRX_REVERSIBLE_POLL_INTERVAL);
